...
# Salvar os resultados em um arquivo na pasta correspondente
pasta_resultado = os.path.join("resultados", periodo)
os.makedirs(pasta_resultado, exist_ok=True)
resultado_filename = os.path.join(pasta_resultado, f"resultado_{periodo}.txt")
with open(resultado_filename, 'w') as resultado_file:
    for nome, status in presenca.items():
        resultado_file.write(f"{nome}: {status}\n")

print(f"Resultado salvo em {resultado_filename}")